...
func _create_animation(idx : int, animation_type : ANIMATION_TYPE) -> void:
	...
	if allow_loop && display_loop:
		...
		if absi(_scroll_value - desired_scroll) > (max_scroll >> 1):
...
func _adjust_children() -> void:
	...
	if display_loop:
		...
		for item : int in range:
			...
			item_info.node.z_index = max_local_offset - absi(local_offset)
		...
		if display_range == -1:
			...
			range = range(maxi(0, index - display_range), mini(_item_count, index + display_range + 1))
		...
		for item : int in range:
			...
			fit_child_in_rect(item_info.node, rect)
			item_info.node.visible = true
			item_info.node.z_index = max_local_offset - absi(local_index)
			_on_item_progress(item_info.node, local_scroll, scroll, item, local_index)
